...
const thumbnail=p=>p.image.replace(/assets\/(p\d+)\.jpg(?:\?.*)?$/,'assets/thumbs/$1.webp?v=20260917');
function render(){const list=filteredProducts(),q=$('search').value.trim();$('clear').hidden=!q;$('result-title').textContent=q?'搜索结果':selected;$('result-count').textContent=`${q?'“'+q+'” · ':''}${selected==='全部产品'?'':catLabel(selected)+' · '}共 ${list.length} 款产品`;$('empty').hidden=list.length>0;$('grid').innerHTML=list.map((p,i)=>`<article class="product"><button class="product-button" data-product="${p.id}" aria-label="查看${esc(p.name)}的介绍"><img src="${thumbnail(p)}" alt="${esc(p.name)}产品图册" loading="${i<4?'eager':'lazy'}" decoding="async" width="720" height="960"><span class="product-body"><span class="tag">${catLabel(p.category)}</span><span class="product-name">${esc(p.name)}</span><span class="product-desc">${esc(p.description)}</span><span class="product-foot"><span class="price ${p.price===null?'pending':''}">${p.price===null?'价格待核实':'<small>¥</small> '+p.price}</span><span class="view">详情 <svg class="ui-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 12h14m-5-5 5 5-5 5"/></svg></span></span></span></button><button class="card-add" data-add="${p.id}" aria-label="将${esc(p.name)}加入选购清单">＋ 加入清单</button></article>`).join('');}
function showProduct(id){const p=PRODUCTS.find(p=>p.id===id);if(!p)return;$('detail-title').textContent=p.name;$('detail-category').textContent=p.category;$('detail-price').textContent=p.price===null?'价格待核实':'¥ '+p.price;$('detail-description').textContent=p.description;$('detail-image').src=p.image;$('detail-image').alt=p.name+'产品图册';if(!$('detail').open)$('detail').showModal();$('detail').scrollTop=0;document.body.classList.add('modal-open');window.catalogFeatures?.onProductOpen(p);}
